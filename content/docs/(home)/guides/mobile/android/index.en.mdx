...
The **elepay Android SDK** integrates stera smart one into Android apps.<br />
This guide explains in detail how to connect your Android app with the stera smart one SDK.
...
You need an stera smart one account to request an `appKey`.
...
> Notes
>
> 1. When specifying an stera smart one theme, the app’s configuration may change, which can cause the `Activity` that invokes the stera smart one payment flow to restart.  
>    To avoid an Activity restart, add `uiMode` to `android:configChanges` of the Activity that invokes stera smart one in `AndroidManifest.xml`.
