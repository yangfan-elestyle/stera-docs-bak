...
The elepay server returns a Refund object in response to the request.<br />
If the request is accepted, the `failure_msg` field of the Refund object is null.

## 3. Receive webhook event

If the refund succeeds, elepay sends a `refund.succeeded` event notification to the URL configured for your webhook.
