...
<EText name="elepay" /> API のレスポンスに含まれる `errorCode`
...
- **S** : System エラー — <EText name="elepay" /> や決済事業者側で発生した内部エラー
...
| フィールド      | 説明                                                                                                                                                                             |
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `errorCode`     | 上表で定義された <EText name="elepay" /> 標準エラーコード。ビジネスロジックの分岐はこの値で行ってください。                                                                      |
...
| `providerError` | 決済事業者（Stripe / PayPay / GMO など）から返却された生のエラー情報を透過的に返します。<EText name="elepay" /> が網羅していない事業者固有のエラーを識別する際にご利用ください。 |

### `providerError` について

`providerError.code` と `providerError.message` は **決済事業者側の定義** であり、事業者ごとに形式が異なります。長期運用では `errorCode`（<EText name="elepay" /> 標準）で分岐することを推奨しますが、以下のようなケースでは `providerError` が有用です。

- 新しい事業者固有エラーが <EText name="elepay" /> 側でまだ標準コードに変換されていない場合
