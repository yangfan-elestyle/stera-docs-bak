...
The **elepay iOS SDK** is an SDK for integrating stera smart one into iOS apps.<br />
...
> 📘 From elepay iOS SDK 5.0, SPM (Swift Package Manager) is the default distribution method. If you have requirements for other integration channels, please contact the stera smart one team.
...
See “[Obtaining URL Scheme for iOS / Android SDK](/guides/mobile/url-scheme)” for how to get the stera smart one‑specific URL Scheme.
...
>
> Replace `ELEPAY_APP_PUBLIC_KEY` with the one issued on the [stera smart one dashboard](https://dashboard.sterasmartone.com/).
...
>
> In‑app payment results should only be used for user‑facing confirmation. Always verify the actual charge result on your server using the callback API and the data sent by the stera smart one servers. _See the elepay API guide for details._
...
2. Upload your Apple Pay certificate to [“stera smart one Dashboard” → “Developer Settings” → “Apple Pay”](https://dashboard.sterasmartone.com/).  
